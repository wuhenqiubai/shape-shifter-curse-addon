 ...
 * <p>返回 {@link InteractionResult#FAIL} 取消交互（等价于原 mixin 在 interactMob 返回 PASS 拦下开交易），
 * 客户端与服务端均生效（客户端拦下即不发交互包，服务端兜底），与原 mixin 双端行为一致。
 */
public final class VillagerTradeGuardHandler {

	private static final ResourceLocation FAMILIAR_FOX_SP = ResourceLocation.fromNamespaceAndPath("my_addon", "form_familiar_fox_sp");
	...
	public static void register() {
		UseEntityCallback.EVENT.register((player, world, hand, entity, hitResult) -> {
			...
			try {
				...
				ResourceLocation formId = component.nowForm.getFormID();
				// 进化使魔：任何右键村民都禁止交易
				if (FormIdentifiers.UPGRADE_FAMILIAR_FOX.equals(formId)) {
					return InteractionResult.FAIL;
				}
				// 灵界之主：潜行右键村民时禁止交易
				if (player.isShiftKeyDown() && FAMILIAR_FOX_SP.equals(formId)) {
					return InteractionResult.FAIL;
				}
