...
/**
 * 客户端独占工具，专门承载所有 {@link Minecraft} 相关的冷却 UI 计算。
...
public final class LifesavingCatTailClient {
	...
	public static int getCooldownRemaining(Item item, int maxCooldown) {
		...
		float progress = client.player.getCooldowns().getCooldownPercent(item, 0.0f);
	...
	public static boolean isOnCooldown(Item item) {
		...
		return client.player.getCooldowns().isOnCooldown(item);
	}
}
